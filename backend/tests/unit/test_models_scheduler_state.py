...
def test_scheduler_state_timing_fields(session):
    ...
    last_scan = utcnow() - timedelta(hours=1)
    next_scan = utcnow() + timedelta(hours=1)
...
def test_has_run_property_has_ran(session):
    ...
    state = SchedulerState(
        id=1,
        last_scan_at=utcnow() - timedelta(hours=1),
    )
    session.add(state)
    session.commit()
...
def test_time_since_last_scan_property(session):
    ...
    last_scan = utcnow() - timedelta(hours=2)
...
def test_time_until_next_scan_negative(session):
    ...
    past_scan = utcnow() - timedelta(hours=1)
...
def test_scheduler_state_complete_lifecycle(session):
    ...
    state.next_scan_at = utcnow() + timedelta(minutes=30)
    state.total_scans = 1
